...
本节从产品与少量技术结合的视角，预演评审/用户可能追问的问题并给出应答，补充各功能模块的产品层面说明。

### 25.1 AIOS 桌宠事件总线

**Q：已经有 Redis 做消息通信了，为什么还要引入 RabbitMQ？是不是过度设计？**

两者定位不同。Redis Pub/Sub 是"服务器内部"的消息分发（群消息广播、流式 chunk），订阅者断线即丢消息。AIOS 事件总线是"跨服务→桌面端"的可靠推送，RabbitMQ 的 `amq.topic` + MQTT 插件能保证：①多服务都能往同一交换机发；②桌宠离线时消息可留存；③用 topic 路由键按用户维度精准投递（`aios.events.user_{id}.#`）。用 Redis 做这件事要么得轮询、要么得每用户一个频道，扩展性差。

**Q：@AiosNotify 用 AOP 注解，业务无感知——万一某个方法不该发通知怎么办？**

注解是 opt-in 的，只有显式加了 `@AiosNotify` 的方法才推送。切面只在方法**正常返回**后执行，异常时不发成功通知，避免误导用户。

### 25.2 桌面宠物与微服务大管家

**Q：桌宠监控微服务健康，对用户有什么实际价值？不就是个花哨的功能吗？**

价值有三层：①**故障可见性**——三个微服务任一挂掉，用户发起 AI 对话/文档处理/编程会直接报错，桌宠提前弹气泡告知"沙盒服务异常，编程功能暂不可用"，比用户点下去再失败体验好；②**降低客服成本**——用户不用猜是哪里坏了；③**伴学沉浸感**——Idle 时推闪卡，把"等待时间"变成"学习时间"，这是产品的差异化记忆点。

**Q：闪卡"碎碎念"会不会打扰用户？**

有频率控制，只在桌宠 Idle 状态触发，且可配置间隔。闪卡走间隔重复算法，只在卡片到期时才推，不是随机骚扰。

### 25.3 职业辅导套件

**Q：职业建议是 AI 生成的，怎么保证不是"正确的废话"？**

建议不是凭空生成，底座是 Neo4j 知识图谱里用户的**真实做题数据**（四科掌握度、薄弱点、活跃度）。AI 拿到的是结构化画像，输出绑定在具体数据上（如"你的操作系统掌握度 42%，低于平均，建议优先复习进程调度"），而非泛泛而谈。

**Q：职业辅导和考情大屏数据是不是重复了？**

数据源相同（都是 Neo4j 认知图谱），但**视角不同**：考情大屏是"学习视角"（你学得怎么样），职业辅导是"发展视角"（你的能力结构适合什么方向、该补什么）。前者面向学生本人，后者还能模拟面试官。

### 25.4 Butcanthic 闪卡 / GraphRAG / Excel 对话

**Q：闪卡和文档处理是什么关系？用户上传一个 PDF 就自动出闪卡吗？**

对。文档进 Celery 流水线后，`kb_processor` 在分块+向量化时，同时调 LLM 抽取 Q&A 对写入 `flashcard` 表。用户无需额外操作，上传即生成。闪卡再通过间隔重复算法调度复习，并同步给桌宠推送。

**Q：GraphRAG 听起来很美，但 Neo4j 又是一个依赖，投入产出比合理吗？**

GraphRAG 不是替代向量检索，而是**增强**。纯向量检索在"实体间关系"类问题上弱（比如"A 和 B 谁先提出"），图谱能补这个短板。四路融合（向量+BM25+图谱+Reranker）后召回率明显提升。若不想部署 Neo4j，GraphRAG 可降级关闭，系统仍能跑三路检索，这是可选增强而非硬依赖。

**Q：Excel 对话用 DuckDB，为什么不用 pandas 直接查？**

pandas 全量加载到内存，大 Excel（几十万行）会 OOM。DuckDB 是列式 OLAP 引擎，能直接对文件做 SQL 查询，内存占用低且支持复杂聚合。用户问的是自然语言，Agent 转成 SQL 后 DuckDB 执行更自然。

### 25.5 Eruitah 绝对网关 / 代码图谱 / 进阶实验引擎

**Q：绝对网关的 TOUR/AMBIGUOUS/CODE 意图判定，准确率怎么样？判定错了怎么办？**

判定由 Supervisor（CTO 路由器）用 LLM 做，非关键词匹配，准确率较高。即便判错也有兜底：AMBIGUOUS 意图会让 Agent 反问澄清而非直接干活，错了最多多一轮对话；CODE 被误判成 TOUR，用户一句"我不是要看导览"就能拉回。设计上**宁可判暧昧也不要乱编码**。

**Q：代码图谱的"变更影响传染分析"对用户实际有什么用？**

改一个函数可能炸一片。传染分析能告诉用户"你改了 A，会波及 B/C/D 三个调用方"，在 Agent 改代码前就给出影响范围，避免改了东墙塌西墙。这是把"资深工程师的直觉"产品化给初级开发者。

**Q：影子沙盒、忒修斯之船这些"进阶实验引擎"，是不是 demo 性质的噱头？真的能用吗？**

坦诚说，这部分是**研究性探索**，不是生产稳定功能。影子沙盒（投机执行）和忒修斯（自我重写）目前更像是技术预研，验证"Agent 能否预判和自我进化"的可行性。产品上不作为卖点承诺，但在文档里保留是为了：①展示技术储备深度；②给后续迭代留接口。建议评审时标注为"实验性功能"。

**Q：成本护栏 5 美元上限，这个数字怎么来的？会不会任务做到一半被砍？**

5 美元是单会话默认上限，可按需调。被砍不是"硬停"，而是 Agent 收到预算告警后优先收尾当前步骤再停止，不会留半截烂代码。这个机制主要防 Agent 陷入死循环烧钱，正常任务远到不了上限。
